...
export const constructCampaignDetails = (chainId: ChainId, address: string, data: EscrowCreateDto, tokenDecimals: number, fees: { exchangeOracleFee: bigint, recordingOracleFee: bigint, reputationOracleFee: bigint }) => {
  ...
  return {
    ...
    launcher: address,
    exchange_oracle: '',
    exchange_oracle_fee: Number(fees.exchangeOracleFee),
    recording_oracle: '',
    recording_oracle_fee: Number(fees.recordingOracleFee),
    reputation_oracle: '',
    reputation_oracle_fee: Number(fees.reputationOracleFee),
    amount_paid: '0',
    daily_paid_amounts: [],
  }
}
