...
const DeleteApiKeyModal: FC<Props> = ({ open, onClose, exchangeName }) => {
  const { mutate: deleteApiKey, reset: resetMutation, isPending, isIdle, isSuccess, isError } = useDeleteApiKeyByExchange(exchangeName);
  ...
  return (
    ...
    >
      <Stack textAlign="center" alignItems="center">
        <Typography variant="h4" py={1} mb={2}>Delete API key</Typography>
        {isPending && <ModalLoading />}
        {isIdle && (
          <>
            <Typography variant="subtitle2" py={1} mb={4} textAlign="center">
              You are about to delete an API KEY.<br />This action can&apos;t be undone, do you want to continue?
            </Typography>
        ...
        {isError && <ModalError />}
        {!isIdle && (
          <Button size="large" variant="contained" disabled={isPending} onClick={handleClose}>
            Close
          </Button>
        )}
      </Stack>
